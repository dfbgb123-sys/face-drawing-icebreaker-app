...
10. ⬜ 클라우드 서버 실제 배포(Render/Railway 등) 및 `PUBLIC_URL` 설정 — **최종 배포 시 반드시 재확인**: 단일 인스턴스가 아니라 **여러 개의 독립된 인스턴스를 각각 별도 서비스로 배포**할 것 (Claude와 논의한 샤딩 방식). 세션이 처음 만들어질 때 인스턴스 하나에 고정되는 구조라 Redis adapter나 그림 저장을 S3로 옮길 필요는 없음 — 로컬 디스크 저장 그대로 유지. 단, 호스트 앱이 세션 생성 시 여러 인스턴스 중 하나를 고르는 로직(알려진 인스턴스 주소 목록에서 랜덤/라운드로빈 선택 — `HostScreen.tsx`의 `createSession` 호출부에 추가 예정)이 먼저 붙어 있어야 이 배포가 의미가 있음. 참고: RN 앱은 `ServerSetupScreen`에서 서버 주소를 직접 입력받아 AsyncStorage에 저장하는 방식으로 구현했음 — `PUBLIC_URL` 배포 후에는 그 주소를 입력하면 됨. 로컬 개발 중엔 같은 와이파이의 PC LAN IP(`http://192.168.x.x:3000`)를 입력하면 됨.
11. ✅ 실기기로 전체 플레이 사이클 검증 완료 (아래 "11번 항목 상세" 참고). 삼성 갤럭시(SM-S911N) 실기기에 USB로 설치해 참가자 화면(1:1/바톤터치/1:多 세 모드 전부 — 그리기 캔버스, 캔버스 이어그리기, 추측/정답공개 패널 포함)과 진행자 화면(설정/로비/진행/결과 전체 흐름, 실시간 로스터, 강제진행, 세션종료)을 실제로 눌러가며 끝까지 검증함. 크래시·에러 로그 없음. 남은 건 폰 2대 이상 물리적 동시접속과 클라우드 배포 환경 검증뿐(아래 참고).
...
### 진행자(Host) 화면 실기기 조작 검증 (3차 세션)

이번엔 폰을 진행자로, 나머지 참가자는 `socket.io-client` 임시 스크립트(`test-hostflow-tmp.js`, 테스트 후 삭제)로 실제 접속시켜서 `HostScreen`을 손가락으로 직접 눌러가며 전체 흐름을 검증함. 좌표는 스크린샷 배율 추정 대신 `adb shell uiautomator dump`로 정확한 버튼 bounds를 뽑아서 탭함 (배율 추정 탭이 몇 차례 빗나갔던 것에서 얻은 교훈).

**확인된 것 (전부 정상)**:
- **SetupView**: 모드 버튼(1:1/바톤터치/1:多) 탭 시 아이콘·설명·힌트 텍스트 즉시 갱신, 참가자 이름 textarea 입력 시 "참가자 N명" 카운트 실시간 반영, 라운드 시간 입력, "시작하기" 탭 시 실제로 세션 생성(REST) 성공.
- **LobbyView**: QR 코드 이미지, 입장 주소(`http://192.168.45.231:3000/`), 선택된 모드 라벨 정상 표시. 실제 참가자 2명이 소켓으로 접속하자 로스터의 연결 표시 점이 회색→초록으로 **실시간** 전환되고 "제외" 버튼이 사라짐 (claimed 참가자는 제외 불가 로직 확인). **"제외" 버튼 탭 시 해당 참가자가 로스터에서 즉시 제거**되는 것도 확인.
- **ProgressView**: "시작" 탭 → 대기(intermission) 상태 "잠시 후 라운드 1/1 시작" + 카운트다운 정상 표시 → **"다음 라운드로 넘기기"(강제 진행) 버튼 탭 시 대기를 건너뛰고 바로 그리기 라운드 시작**, 두 참가자가 실제로 제출하자 라운드 종료 → 결과 화면까지 자동 전환.
- **ResultsView / EndedView**: "세션 완전히 종료" 버튼 → 네이티브 확인창(Alert, "세션을 종료할까요? 되돌릴 수 없어요.") → 확인 시 "세션이 종료됐어요" 화면 → "새 세션 만들기" 링크로 SetupView 복귀까지 정상.
- 앱을 재시작(force-stop 후 재실행)해도 이미 진행 중인 세션의 상태(이 경우 results)를 정확히 다시 감지해서 알맞은 화면을 보여줌.

**발견한 사소한 제약 (버그 아님, 원본 웹 버전과 동일한 동작)**: 진행자 앱이 세션 생성 *이후*에 재시작되면(this 세션에서는 세션을 REST로 직접 만들어서 재현), `host:join`으로 재연결했을 때 서버 스냅샷에 QR 코드/입장 주소가 포함되지 않아 로비 화면에 QR 이미지와 주소가 빈 채로 나옴 (로스터·시작 버튼 등 기능은 정상). 원본 웹 버전의 `host.js`도 `hostJoinAndRender`에서 동일하게 재설정하지 않으므로 포팅 과정에서 생긴 회귀가 아니라 기존 설계상 제약임. 실제 사용 흐름(진행자가 세션 만든 뒤 앱을 재시작하지 않음)에서는 문제되지 않음.

### 아직 검증 못 한 것 (다음에 이어서 할 것)
- **지우개 도구**, **전체 지우기 버튼**은 육안으로 버튼만 확인했고 실제 지워지는 동작은 안 눌러봄.
- 두 명 이상이 **서로 다른 물리적 화면**(폰 2대 이상)으로 동시에 실기기 접속하는 시나리오는 테스트 안 함 (이번 세션들은 폰 1대 + `socket.io-client` 스크립트로 나머지를 실제 소켓 연결로 대신함 — 서버 관점에서는 동시접속이 맞지만, 물리적으로 다른 기기·화면에서의 검증은 아님).
