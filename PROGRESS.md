...
**주의할 점 (버그는 아니지만 기록)**: 바톤 모드 테스트 중, 제가 직접 조작하지 않은 라운드에서 캔버스에 알 수 없는 낙서가 그려져 서버에 제출된 경우가 한 번 있었음 (서버에 저장된 PNG 파일을 직접 받아 확인해서, 화면 표시 오류가 아니라 실제로 제출된 내용인 걸 확인함). 원인을 특정하진 못했고, 재현도 안 됨 — 이후 1:多 모드 테스트에서는 같은 현상이 없었음. 라운드 길이를 매우 짧게(15~30초) 잡고 여러 tool 호출을 빠르게 이어서 실행하는 테스트 방식 자체의 타이밍 이슈였을 가능성이 높다고 판단했지만, 만약 실제 사용 중 이런 현상이 재현되면 제스처 핸들러가 화면 전환 중 이전 터치 상태를 남기는 문제일 수 있어 다시 조사해볼 것.
